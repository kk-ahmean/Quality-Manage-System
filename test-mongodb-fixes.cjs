...
async function testMongoDBFixes() {
  const browser = await chromium.launch({ headless: false });
  const context = await browser.newContext();
  const page = await context.newPage();

  try {
    console.log('开始测试MongoDB模式下的修复效果...');
    ...
    // 截图保存错误状态
    await page.screenshot({ path: 'mongodb-fixes-error.png', fullPage: true });
    console.log('已保存错误截图: mongodb-fixes-error.png');
  } finally {
    await browser.close();
